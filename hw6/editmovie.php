...
//check if we already have a player with the same name, country, and club as the one we are adding/processing and is not the one we are processing.
if($isComplete){
    // set up a query to check if this player is already in the database
    $query = "SELECT title FROM movies WHERE title ='$title' AND director = '$director' AND reldate = '$reldate' AND id<>$id";
...
if($isComplete){
    ...
    if (nTuples($result) == 0){
        $isComplete = false;
        $errorMessage .= "The id $id does not correspond to any movies in the movies table. "; 
    }
}


// if we get this far and $isComplete is true it means we should edit the movie into the database.
if ($isComplete){
    
    // we will setup the insert statement to add this new record into the database
    $updatequery = "UPDATE movies SET title='$title', director='$director', reldate='$reldate', video='$video' WHERE id=$id ";
